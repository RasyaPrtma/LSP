...
const Home = () => {
    const { article,kategori, getImage, handleSearch,handleFilter,handleSort} = useArticle();
    const [imageUrls, setImageUrls] = useState([]);

    useEffect(() => {
        if (article.length > 0) {
            const fetchArticleImages = async () => {
                const urls = await Promise.all(article.map(async (val) => {
            ...
            <section className='article'>
                <header className='mt-[10rem] flex gap-[10px]'>
                    <div className='relative w-[30vw]'>
                        <input onKeyUp={(e) => handleSearch(e.target.value)} className='relative w-[100%] rounded-[20px] outline-none border-none py-[5px] pl-[10px] font-medium text-[rgba(0,0,0,.7)]' type="text" />
                        <i className=" absolute top-[9px] z-[10] right-[15px] fa-solid fa-magnifying-glass"></i>
                    </div>
                    <select onChange={(e) => handleFilter(e.target.value)} className=' rounded-sm outline-none border-none'>
                    ...
                    <select onChange={(e) => handleSort(e.target.value)} className=' rounded-sm outline-none border-none text-center'>
