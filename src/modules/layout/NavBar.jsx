 ...
 */
const NavBar = () => {
  const { doLogin, doRegist, doLogout, IsLogged, nameUser } = useAuth();
    ...
    <>
      <header className="w-[100%] py-[1.8em] fixed top-[0] z-[100] left-0 right-0">
        <nav className="flex justify-between items-center mx-[2.5em]">
          <NavLink to={"/"} className="text-[1.5em] font-bold text-[#F8F8F8]">
            ARTICULATE
          </NavLink>
          {IsLogged ? (
            <>
      ...
      {loginClick ? (
        <form
          ...
          className="fixed top-[5vw] right-[5vw] z-[20] bg-[rgba(255,255,255,0.5)] w-[300px] h-[300px] rounded-md flex justify-center flex-col items-center"
        >
          <span
            className="cursor-pointer absolute top-[20px] left-[25px] text-[14px] text-[rgba(255,255,255,0.8)]"
            onClick={handleClose}
          >
            <i className="fa-solid fa-x"></i>
          </span>
          <h1 className="text-[1.4em] font-bold text-[rgba(255,255,255,0.8)]">
            Login
          </h1>
          <div className="flex flex-col mb-[1em]">
            ...
            <input
              ...
              type="email"
              id="email"
              className="w-[250px] rounded-sm border-[none] outline-none pl-2 text-[14px] py-[5px] text-[rgba(0,0,0,0.5)]"
              required
            />
          </div>
          <div className="flex flex-col mb-[1em]">
          ...
          <button
            type="submit"
            className="bg-[rgba(0,0,0,0.2)] text-[rgba(255,255,255,0.8)] text-[14px] px-[15px] py-[6px] rounded-md"
          >
            Submit
          </button>
        </form>
      ...
      {registClick ? (
        <form
          onSubmit={(e) => handleSubmitRegist(e)}
          className="fixed top-[5vw] right-[5vw] z-[20] bg-[rgba(255,255,255,0.5)] w-[350px] h-[400px] rounded-md flex justify-center flex-col items-center"
        >
          <span
            className="cursor-pointer absolute top-[20px] left-[25px] text-[14px] text-[rgba(255,255,255,0.8)]"
            onClick={handleClose}
